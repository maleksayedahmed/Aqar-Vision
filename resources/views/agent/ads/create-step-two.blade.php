...
@section('title', __('common.create_ad') . ' - ' . __('common.step_2'))
    ...
    <main class="bg-[rgba(250,250,250,1)] px-4 lg:px-20 pt-6 pb-11">
        ...
        <form method="POST" action="{{ route('agent.ads.store') }}" enctype="multipart/form-data">
            ...
            <section class="">
                <div class="flex w-full flex-col items-stretch gap-y-4 lg:flex-row lg:items-center lg:justify-between py-4"
                    ...
                    <div class="hidden md:flex items-center gap-x-[27px]">
                        <span
                            class="font-medium text-[16px] text-[rgba(181,183,191,1)]">{{ __('common.property_data') }}</span>
                        <div class="flex items-center gap-x-2">
                            <img src="{{ asset('images/rode.svg') }}">
                        </div>
                        <span
                            class="font-medium text-[16px] text-[rgba(48,62,124,1)]">{{ __('common.property_documents') }}</span>
                    </div>

                    <div class="relative w-full lg:w-[264px]">
                        <button type="button"
                            class="flex items-center justify-between bg-[rgba(0,0,0,0.02)] w-full h-[53px] px-4 py-2.5 rounded-xl text-lg lg:text-[20px] font-medium text-black">
                            <div class="flex items-center gap-x-2">
                    ...
                    <div class="text-gray-800 space-y-4 text-[16.3px] font-medium">
                        ...
                        <div>
                            ...
                            <ul class="space-y-2 pr-4 md:pr-6 text-[rgba(48,62,124,1)]">
                                ...
                                <li>{{ __('common.images_guideline_lighting_no_watermarks') }}</li>
                                <li>{{ __('common.images_guideline_dimensions') }}</li>
                                <li>{{ __('common.images_guideline_max_size', ['size' => 5]) }}</li>
                                <li>{{ __('common.images_guideline_main_image') }}</li>
                            </ul>
                        </div>
                        <div>
                            ...
                            <ul class="space-y-2 pr-4 md:pr-6 text-[rgba(48,62,124,1)]">
                                ...
                                <li>{{ __('common.video_should_be_short') }}</li>
                                <li>{{ __('common.video_quality_sound') }}</li>
                                <li>{{ __('common.video_supported_formats') }}</li>
                                <li>{{ __('common.video_max_size', ['size' => 50]) }}</li>
                            </ul>
                        </div>
                        <p class="text-[rgba(48,62,124,1)]">{{ __('common.media_delete_hint') }}</p>
                    </div>
                </div>

                <div class="mt-8">
                    <div id="drop-zone"
                        class="flex h-36 w-full items-center justify-center rounded-xl border-2 border-dashed border-gray-300 transition-colors">
                        <label for="file-upload" class="cursor-pointer text-center text-base text-gray-400 sm:text-lg">
                            {{ __('common.drag_and_drop_here_or') }}
                            <span
                                class="font-semibold text-sky-500 hover:text-sky-600">{{ __('common.click_to_upload') }}</span>
                    ...
                    <div class="mt-6 border-b border-gray-200">
                        <nav class="-mb-px flex gap-x-4 sm:gap-x-8" aria-label="Tabs">
                            <button type="button" data-type="image"
                                class="tab-btn whitespace-nowrap border-b-2 border-sky-500 px-1 py-4 text-base font-medium text-sky-500 sm:text-lg"
                                aria-current="page">{{ __('common.images') }}</button>
                            <button type="button" data-type="video"
                                class="tab-btn whitespace-nowrap border-b-2 border-transparent px-1 py-4 text-base font-medium text-gray-500 hover:text-gray-700 sm:text-lg">{{ __('common.videos') }}</button>
                        </nav>
                    </div>
            ...
            <div class="mt-8">
                <div class="relative flex items-start gap-x-3" dir="rtl">
                    ...
                    <div class="text-sm leading-6">
                        <label for="terms"
                            class="block text-[14.25px] font-bold text-[rgba(13,18,38,1)]">{{ __('common.confirm_all_info_correct') }}</label>
                        <p class="mt-1 text:[13px] font-medium text-[rgba(13,18,38,1)]">
                            {{ __('common.data_accuracy_notice') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-center mt-12">
                <button type="submit"
                    class="flex items-center justify-center gap-x-2 bg-[rgba(48,62,124,1)] hover:bg-blue-800 text-white text-base sm:text-[19px] font-medium py-3 px-8 sm:px-16 w-full sm:w-auto rounded-lg transition-colors">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            ['dragenter', 'dragover'].forEach(eventName => {
                dropZone.addEventListener(eventName, () => dropZone.classList.add('border-sky-500',
                    'bg-sky-50'), false);
            });

            ['dragleave', 'drop'].forEach(eventName => {
                dropZone.addEventListener(eventName, () => dropZone.classList.remove('border-sky-500',
                    'bg-sky-50'), false);
            });
            ...
            function renderThumbnails() {
                ...
                filesToRender.forEach(file => {
                    ...
                    if (file.type.startsWith('image/')) {
                        ...
                        mediaElement.addEventListener('loadeddata', () => mediaElement
                    .pause()); // Ensure it shows the first frame
                    }
                    ...
                    deleteButton.className =
                        'absolute right-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white transition-colors hover:bg-red-600';
