...
<head>
    ...
    <link rel="icon" type="image/png" href="{{ asset('images/favicon-dark.png') }}"
...
<body class="bg-white">
        ...
        <div id="upgrade-account-modal"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg m-4 relative">
                ...
                <div class="p-6 sm:p-8 flex flex-col items-center max-h-[80vh] overflow-y-auto">
                    ...
                    <!-- Close Button -->
                    <button id="close-upgrade-modal"
                        class="absolute top-4 left-4 w-6 h-6 bg-gray-200 text-gray-800 rounded-full flex items-center justify-center hover:bg-gray-300 transition-colors"
                    ...
                    <!-- Heading -->
                    <h2 class="text-lg font-bold text-gray-800 my-3">{{ __('common.upgrade_account_title') }}</h2>
                    <p class="text-sm text-gray-500 mb-4 text-center">{{ __('common.upgrade_select_role') }}</p>

                    <!-- Upgrade Form -->
                    <form id="upgrade-request-form" action="{{ route('user.upgrade.request') }}" method="POST"
                        ...
                        <fieldset class="space-y-4">
                            ...
                            <!-- Agent Option -->
                            <div class="relative">
                                <input class="sr-only peer" type="radio" value="agent" name="requested_role"
                                    id="role_agent" checked>
                                <label
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-lg font-bold">{{ __('common.agent_role_label') }}</span>
                                        <span class="text-sm text-gray-500 mt-1">{{ __('common.agent_role_desc') }}</span>
                                    </div>
                                </label>
                            </div>
                            ...
                            <!-- Agency Option -->
                            <div class="relative">
                                <input class="sr-only peer" type="radio" value="agency" name="requested_role"
                                    id="role_agency">
                                <label
                                    class="flex p-5 bg-white border border-gray-300 rounded-lg cursor-pointer focus:outline-none hover:bg-gray-50 peer-checked:ring-blue-500 peer-checked:ring-2 peer-checked:border-transparent"
                                    for="role_agency">
                                    <div class="flex flex-col">
                                        <span class="text-lg font-bold">{{ __('common.agency_role_label') }}</span>
                                        <span class="text-sm text-gray-500 mt-1">{{ __('common.agency_role_desc') }}</span>
                                    </div>
                                </label>
                            </div>
                        </fieldset>
                        ...
                        <div id="fal-license-field" class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label for="upgrade_license_issue_date"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.license_issue_date') }}
                                ...
                                <div class="space-y-2">
                                    <label for="upgrade_license_expiry_date"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.license_expiry_date') }}
                                        ({{ __('common.optional') }})</label>
                        ...
                        <div id="agency-fields" class="space-y-4 hidden">
                            <div class="space-y-2">
                                <label for="agency_name"
                                    class="block text-sm font-medium text-gray-700">{{ __('common.agency_name_label') }}</label>
                                <input type="text" name="agency_name" id="agency_name"
                                    placeholder="{{ __('common.enter_company_name') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label for="agency_type_id"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.company_type_label') }}</label>
                                    <select name="agency_type_id" id="agency_type_id"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                        <option value="">{{ __('common.choose_company_type') }}</option>
                                ...
                                <div class="space-y-2">
                                    <label for="commercial_register_number"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.commercial_register') }}</label>
                                    <input type="text" name="commercial_register_number"
                                        id="commercial_register_number"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                        placeholder="{{ __('common.commercial_register_placeholder') }}">
                                </div>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label for="commercial_issue_date"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.commercial_issue_date') }}</label>
                                    <input type="date" name="commercial_issue_date" id="commercial_issue_date"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                </div>

                                <div class="space-y-2">
                                    <label for="commercial_expiry_date"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.commercial_expiry_date') }}</label>
                                    <input type="date" name="commercial_expiry_date" id="commercial_expiry_date"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                    <label for="tax_issue_date"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.tax_issue_date') }}</label>
                                    <input type="date" name="tax_issue_date" id="tax_issue_date"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label for="tax_expiry_date"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.tax_expiry_date') }}</label>
                                ...
                                <div class="space-y-2">
                                    <label for="address"
                                        class="block text-sm font-medium text-gray-700">{{ __('common.office_address_label') }}</label>
                                    <input type="text" name="address" id="address"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                        placeholder="{{ __('common.office_address_placeholder') }}">
                                </div>
                            </div>
                        ...
                        <div id="upgrade-form-error"
                            class="hidden text-right text-red-500 text-sm p-2 bg-red-50 rounded-lg"></div>
                        <div class="pt-3">
                            <button type="submit"
                                ...
                                <svg class="spinner hidden animate-spin -ml-1 mr-3 h-5 w-5 text-white"
                                ...
                                <span class="button-text">{{ __('common.submit_upgrade_request') }}</span>
                            </button>
                        </div>
                    </form>
                </div>
    ...
    {{-- Success Alert Modal --}}
    {{-- ================================================= --}}
    <div id="success-alert-modal"
        class="hidden fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-60 transition-opacity duration-300">
        <div
            class="bg-white rounded-2xl shadow-xl w-full max-w-sm m-4 pt-10 pb-8 relative flex flex-col items-center transform transition-all scale-95 opacity-0">
            ...
            <!-- Heading -->
            <h2 id="success-alert-title" class="text-2xl font-bold text-gray-800 my-4">
                {{ __('common.success_title') }}</h2>
            <p id="success-alert-message" class="text-base text-gray-500 mb-8 text-center px-4">
                {{ __('common.request_sent_successfully') }}</p>

            <button id="close-success-alert"
                class="bg-[#303F7C] text-white font-bold py-3 px-12 rounded-lg hover:bg-opacity-90 transition-colors shadow-md">
                {{ __('common.ok') }}
            </button>
        </div>
    </div>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (switchToLoginEmailBtn) switchToLoginEmailBtn.addEventListener('click', () => showModal(
                loginEmailModal));
