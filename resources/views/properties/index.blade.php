...
@section('title', __('common.search_results_title'))
        ...
        <section class="w-full bg-white">
                    ...
                    <div class="flex flex-col-reverse items-center justify-between gap-4">
                        ...
                        <div id="view-toggle" class="inline-flex p-1 bg-gray-200 rounded-full self-end">
                            <span
                                ...
                                <img src="{{ asset('images/list.svg') }}"><span>{{ __('common.list_view') }}</span>
                            </span>
                            <a href="{{ route('properties.map', request()->query()) }}"
                                class="view-toggle-btn flex items-center gap-2 p-3 text-sm font-semibold rounded-full text-gray-600">
                                <img src="{{ asset('images/map.svg') }}"><span>{{ __('common.map_view') }}</span>
                            </a>
                        </div>
                    </div>
            ...
            <div class="border-t border-gray-200 mt-6 pt-6 px-3">
                <div class="flex items-end justify-between">
                    <div>
                        <p class="text-sm text-gray-500 mb-1 px-3">{{ __('common.search_results_label') }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 px-4 gap-6 mt-8 place-items-center">
                                ...
                                <div class="border-t border-gray-100 pt-5 mt-5 flex justify-between items-center">
                                    <p class="text-lg font-bold text-black flex items-baseline gap-1">
                                        ...
                                        <span class="flex items-center">
                                            <span class="text-sm font-medium text-gray-500">{{ __('common.sar') }}</span>
                                            @if ($ad->listing_purpose == 'rent')
                                                <span
                                                    class="text-sm font-bold text-black">{{ __('common.per_month') }}</span>
                                            @endif
                                            <span>
                                    </p>
                ...
                @empty
                    <p class="col-span-4 text-center text-gray-500 py-16">{{ __('common.no_matching_properties') }}</p>
                @endforelse
            </div>
                    ...
                    <nav class="flex items-center gap-3 flex-row-reverse" aria-label="Pagination">
                        @if (!$ads->onFirstPage())
                            <a href="{{ $ads->previousPageUrl() }}"
                                class="text-sm text-gray-600 hover:text-gray-900">{{ __('common.previous') }}</a>
                        @endif
                        @foreach ($ads->links()->elements as $element)
                            @if (is_string($element))
                                <span
                                    class="flex items-center justify-center w-10 h-10 rounded-full text-gray-500">{{ $element }}</span>
                            @endif
                            @if (is_array($element))
                                @foreach ($element as $page => $url)
                                    @if ($page == $ads->currentPage())
                                        <span
                                            class="flex items-center justify-center w-10 h-10 rounded-full bg-[rgba(48,63,125,1)] text-white text-sm font-medium"
                                            aria-current="page">{{ $page }}</span>
                                    @else
                                        <a href="{{ $url }}"
                                            class="flex items-center justify-center w-10 h-10 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 text-sm font-medium">{{ $page }}</a>
                                    @endif
                                @endforeach
                            @endif
                        @endforeach
                        @if ($ads->hasMorePages())
                            <a href="{{ $ads->nextPageUrl() }}"
                                class="text-sm text-gray-600 hover:text-gray-900">{{ __('common.next') }}</a>
                        @endif
                    </nav>
                @endif
            </div>
                ...
                menu.querySelectorAll('.select-option').forEach(option => {
                    option.addEventListener('click', event => {
                        ...
                        if (filterName === 'city_id') {
                            const districtHiddenInput = filterForm.querySelector(
                            ...
                            if (districts.length > 0) {
                                districts.forEach(district => {
                                    ...
                                    a.addEventListener('click', (event) => {
                                        ...
                                        districtWrapper.querySelector('.dropdown-menu')
