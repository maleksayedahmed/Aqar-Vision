<div class="bg-white rounded-2xl shadow-sm p-6 sm:p-8" wire:poll.10s>
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">{{ __('common.notifications') }}</h2>
        <button wire:click="deleteAll" wire:confirm="{{ __('common.confirm_delete_all_notifications') }}"
            class="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100">
    ...
    <div class="space-y-4">
        @forelse ($notifications as $notification)
            <div
                class="flex items-start gap-4 p-4 rounded-lg border {{ $notification->read_at ? 'border-gray-100' : 'border-blue-200 bg-blue-50' }}">

                @if ($notification->data['status'] === 'active')
                    <div
                        class="flex-shrink-0 w-10 h-10 bg-green-100 text-green-600 rounded-full flex items-center justify-center">
                ...
                @else
                    <div
                        class="flex-shrink-0 w-10 h-10 bg-red-100 text-red-600 rounded-full flex items-center justify-center">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
