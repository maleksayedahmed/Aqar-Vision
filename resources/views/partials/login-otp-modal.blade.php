...
<div id="otp-modal"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md m-4 pt-12 p-8 relative flex flex-col items-center">
        ...
        <button id="back-to-login-phone-modal" class="absolute top-5 right-4 text-gray-500 hover:text-black"
            aria-label="Go back">
        ...
        <div class="text-center my-6">
            <h2 class="text-[16px] font-bold text-gray-800">
                {{ __('auth.otp_code', [], app()->getLocale()) ?? 'رمز التحقق' }}</h2>
            <p class="text-[9.1px] mt-2 max-w-xs">
                {{ __('auth.enter_otp_sent', [], app()->getLocale()) ?? 'ادخل رمز التحقق المرسل الى رقم هاتفك.' }}</p>
            @if (session('status'))
        ...
        <form action="{{ route('login.phone.verify') }}" method="POST" class="w-full">
            ...
            <div id="otp-inputs" class="flex justify-center gap-3 mb-8" dir="ltr">
                <input type="text" maxlength="1"
                    class="otp-input w-16 h-16 text-center text-2xl font-semibold border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#303F7C]">
                <input type="text" maxlength="1"
                    class="otp-input w-16 h-16 text-center text-2xl font-semibold border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#303F7C]">
                <input type="text" maxlength="1"
                    class="otp-input w-16 h-16 text-center text-2xl font-semibold border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#303F7C]">
            ...
            <div class="pt-3">
                <button type="submit"
                    class="w-full bg-[rgba(48,62,124,1)] text-white font-bold py-3 rounded-lg hover:bg-opacity-90 transition-colors shadow-sm">
                    {{ __('auth.verify', [], app()->getLocale()) ?? 'التحقق' }}
                </button>
            </div>
        </form>
        ...
        <p class="text-sm text-gray-600 pt-8">
            ...
            <button type="button" id="switch-to-signup-from-otp-modal"
